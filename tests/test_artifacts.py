...
from mysorf_base.artifacts import (
...
from mysorf_base.artifacts.backends.gcs import GCSBackend
from mysorf_base.artifacts.backends.local import LocalBackend
from mysorf_base.artifacts.backends.s3 import S3Backend
from mysorf_base.artifacts.core.validate import validate_artifacts_config
...
def test_validate_remote_backend_requires_bucket() -> None:
    cfg = ArtifactsConfig(backend="s3", bucket=None)
...
def test_build_artifact_manager_s3_returns_s3_backend(tmp_path: Path) -> None:
    cfg = ArtifactsConfig(backend="s3", bucket="bucket", versioning_strategy="manual")
    paths = _make_paths_section(tmp_path)
    mgr = build_artifact_manager(cfg, paths)  # type: ignore[arg-type]
    assert isinstance(mgr, S3Backend)


def test_build_artifact_manager_gcs_returns_gcs_backend(tmp_path: Path) -> None:
    cfg = ArtifactsConfig(backend="gcs", bucket="bucket", versioning_strategy="manual")
    paths = _make_paths_section(tmp_path)
    mgr = build_artifact_manager(cfg, paths)  # type: ignore[arg-type]
    assert isinstance(mgr, GCSBackend)
...
def test_null_manager_runs_on_save_hooks(tmp_path: Path) -> None:
    f = tmp_path / "model.pt"
    f.write_bytes(b"data")
    mgr = NullArtifactManager()
    captured: list[ArtifactRecord] = []
    mgr.register_on_save_hook(captured.append)
...
def test_list_artifacts_sorted_by_created_at(tmp_path: Path) -> None:
    ...
    records = mgr.list_artifacts()
    assert all(record.created_at.tzinfo == UTC for record in records)
    for i in range(len(records) - 1):
        assert records[i].created_at <= records[i + 1].created_at


def test_save_record_created_at_is_timezone_aware(tmp_path: Path) -> None:
    src = tmp_path / "model.pt"
    src.write_bytes(b"weights")
    mgr = LocalBackend(str(tmp_path / "arts"), versioning_strategy="manual")
...
def test_local_backend_runs_on_save_hooks(tmp_path: Path) -> None:
    src = tmp_path / "model.pt"
    src.write_bytes(b"weights")
    mgr = LocalBackend(str(tmp_path / "arts"), versioning_strategy="manual")
    captured: list[ArtifactRecord] = []
...
class _FakeS3Client:
    ...
    def upload_file(self, filename: str, bucket: str, key: str) -> None:
        self.storage[f"{bucket}/{key}"] = Path(filename).read_bytes()
    ...
    def list_objects_v2(
                ...
                contents.append(
                    {
                        "Key": key,
                        "Size": len(data),
                        "LastModified": dt.datetime.now(tz=UTC),
                    }
                )
        ...
        is_truncated = start + self.page_size < len(contents)
        if is_truncated:
    ...
    def download_file(self, bucket: str, key: str, filename: str) -> None:
        Path(filename).write_bytes(self.storage[f"{bucket}/{key}"])

    def delete_object(self, Bucket: str, Key: str) -> None:
        self.storage.pop(f"{Bucket}/{Key}")


def test_s3_backend_roundtrip_with_fake_client(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from mysorf_base.artifacts.backends import s3 as s3_module

    fake_client = _FakeS3Client()
    monkeypatch.setattr(s3_module, "_build_s3_client", lambda region: fake_client)
    src = tmp_path / "model.pt"
    src.write_bytes(b"weights")
    mgr = S3Backend(
        "test-bucket",
    ...
    record = mgr.save(src, "model", ArtifactType.CHECKPOINT, version="v1")
    loaded = mgr.load("model", ArtifactType.CHECKPOINT, version="v1")
    listed = mgr.list_artifacts()
    mgr.delete("model", ArtifactType.CHECKPOINT, "v1")

    assert record.path.as_posix().endswith("test-bucket/artifacts/checkpoint/model/v1/model.pt")
    assert loaded.read_bytes() == b"weights"
    assert len(listed) == 1
    with pytest.raises(ArtifactNotFoundError):
        mgr.load("model", ArtifactType.CHECKPOINT, version="v1")


def test_s3_backend_list_artifacts_paginates(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from mysorf_base.artifacts.backends import s3 as s3_module

    fake_client = _FakeS3Client(page_size=2)
    monkeypatch.setattr(s3_module, "_build_s3_client", lambda region: fake_client)
    ...
    mgr.save(src, "a", ArtifactType.CHECKPOINT, version="v1")
    mgr.save(src, "b", ArtifactType.CHECKPOINT, version="v1")
    mgr.save(src, "c", ArtifactType.CHECKPOINT, version="v1")
...
class _FakeGCSBlob:
    def __init__(self, name: str, storage: dict[str, bytes]) -> None:
        self.name = name
        self._storage = storage
        self.size = 0
        self.updated = dt.datetime.now(tz=UTC)
    ...
    def download_to_filename(self, filename: str) -> None:
        Path(filename).write_bytes(self._storage[self.name])

    def delete(self) -> None:
        self._storage.pop(self.name)
...
class _FakeGCSBucket:
    ...
    def blob(self, name: str) -> _FakeGCSBlob:
        return _FakeGCSBlob(name, self._storage)
...
class _FakeGCSClient:
    ...
    def bucket(self, name: str) -> _FakeGCSBucket:
        del name
        return _FakeGCSBucket(self.storage)

    def list_blobs(self, bucket_name: str, prefix: str) -> list[_FakeGCSBlob]:
        del bucket_name
        return [
            _FakeGCSBlob(name, self.storage) for name in self.storage if name.startswith(prefix)
        ]


def test_gcs_backend_roundtrip_with_fake_client(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from mysorf_base.artifacts.backends import gcs as gcs_module

    fake_client = _FakeGCSClient()
    monkeypatch.setattr(gcs_module, "_build_gcs_client", lambda project: fake_client)
    src = tmp_path / "model.pt"
    src.write_bytes(b"weights")
    mgr = GCSBackend(
        "test-bucket",
...
def test_save_uses_staged_write_without_leftover_tmp_dirs(tmp_path: Path) -> None:
    src = tmp_path / "model.pt"
    src.write_bytes(b"weights")
    mgr = LocalBackend(str(tmp_path / "arts"), versioning_strategy="manual")
    ...
    version_root = (tmp_path / "arts" / ArtifactType.CHECKPOINT.value / "model").resolve()
    assert not any(path.name.startswith(".tmp-v1-") for path in version_root.iterdir())
