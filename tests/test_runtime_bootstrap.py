...
- teardown() swallows teardown-step exceptions
...
from mysorf_base.logging import NullLogger
from mysorf_base.profiling import NullProfiler
from mysorf_base.runtime import RuntimeContext, bootstrap, teardown
from mysorf_base.tracking import NullTracker
...
def test_public_api_exports() -> None:
    """RuntimeContext, bootstrap, teardown must be importable from mysorf_base.runtime."""
...
def test_bootstrap_default_returns_runtime_context() -> None:
    ...
    assert ctx.config_hash == sha256_config(ctx.cfg)
...
def test_teardown_calls_tracker_finish() -> None:
    """teardown() must call tracker.finish() and artifact_manager.finalize()."""
    ...
    mock_artifact_manager.finalize.assert_called_once()


def test_teardown_swallows_tracker_exception() -> None:
    """teardown() must not re-raise when teardown steps raise."""
    ctx = bootstrap(["tracking=disabled"])
    mock_tracker = MagicMock()
    mock_tracker.finish.side_effect = RuntimeError("tracker exploded")
    mock_artifact_manager = MagicMock()
    mock_artifact_manager.finalize.side_effect = RuntimeError("artifact finalize exploded")
    ctx2 = RuntimeContext(
        cfg=ctx.cfg,
        run_id=ctx.run_id,
        config_hash=ctx.config_hash,
...
# Feature: runtime-subsystem, Property 5: Teardown swallows teardown-step exceptions
...
def test_property_teardown_swallows_exceptions(message: str) -> None:
    ...
    mock_artifact_manager = MagicMock()
    mock_artifact_manager.finalize.side_effect = Exception(message)
    ctx2 = RuntimeContext(
        cfg=ctx.cfg,
        run_id=ctx.run_id,
        config_hash=ctx.config_hash,
...
def test_teardown_closes_logger_handlers() -> None:
    """teardown() must close logger handlers when the logger exposes them."""
    ctx = bootstrap(["tracking=disabled"])
    mock_tracker = MagicMock()
    mock_artifact_manager = MagicMock()
    handler = MagicMock()
...
def test_context_manager_swallows_tracker_exception_on_exit() -> None:
    """Context manager __exit__ must not re-raise teardown exceptions."""
    ...
    ctx = RuntimeContext(
        ...
        config_hash=ctx_base.config_hash,
        event_bus=ctx_base.event_bus,
...
def test_config_hash_changes_when_config_changes() -> None:
    default_ctx = bootstrap(["tracking=disabled", "logging=disabled"])
    debug_ctx = bootstrap(["tracking=disabled", "logging=disabled", "runtime=debug"])

    assert default_ctx.config_hash != debug_ctx.config_hash


def test_runtime_event_bus_publish_subscribe() -> None:
    ctx = bootstrap(["tracking=disabled", "logging=disabled"])
    events: list[str] = []
    ctx.event_bus.subscribe("run.started", lambda event: events.append(event.name))

    emitted = ctx.event_bus.publish("run.started", {"run_id": ctx.run_id})
...
def test_property_bootstrap_propagates_logger_error() -> None:
    with (
        patch("mysorf_base.logging.build_logger", side_effect=ValueError("bad logger config")),
...
def test_property_bootstrap_propagates_tracker_error() -> None:
    with (
        patch("mysorf_base.tracking.build_tracker", side_effect=ValueError("bad tracker config")),
...
def test_property_bootstrap_propagates_profiler_error() -> None:
    with (
        patch(
            "mysorf_base.profiling.build_profiler", side_effect=ValueError("bad profiler config")
        ),
