...
from mysorf_base.sweeps import (
    CategoricalParam,
    FloatParam,
    IntegerParam,
    SearchSpace,
...
from mysorf_base.sweeps.backends.local import LocalRunner
from mysorf_base.sweeps.backends.null import NullSweepRunner
from mysorf_base.sweeps.backends.wandb import WandbRunner
from mysorf_base.sweeps.core.strategies import GridStrategy, RandomStrategy
from mysorf_base.sweeps.core.validate import validate_sweeps_config
...
def test_validate_disabled_sweeps_allows_null_runner_mode() -> None:
    cfg = SweepsConfig(enabled=False, backend="disabled")
    validate_sweeps_config(cfg)


def test_validate_disabled_requires_disabled_backend() -> None:
    cfg = SweepsConfig(enabled=False, backend="local")
    with pytest.raises(ValueError, match="disabled"):
        validate_sweeps_config(cfg)


def test_build_sweep_runner_returns_null_runner_when_disabled() -> None:
    runner = build_sweep_runner(SweepsConfig(enabled=False, backend="disabled"))
    assert isinstance(runner, NullSweepRunner)
    summary = runner.run([], lambda trial_ctx, params: {"loss": 0.1})
    assert summary.skip_reason == "sweeps disabled by configuration"


def test_run_sweep_returns_empty_summary_when_disabled() -> None:
    ctx = MagicMock()
    ...
    summary = run_sweep(
        space,
        lambda trial_ctx, params: {"loss": 0.1},
        ctx,
        SweepsConfig(enabled=False, backend="disabled"),
    )

    assert summary.results == []
    assert summary.skip_reason == "sweeps disabled by configuration"
    ctx.logger.warning.assert_called_once_with("sweeps disabled by configuration")


# Feature: sweeps-subsystem, Property 13: Validation từ chối backend không hợp lệ
@given(st.text(min_size=1).filter(lambda s: s not in {"disabled", "local", "wandb"}))
@settings(max_examples=100)
...
def test_local_runner_tears_down_each_trial(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(mysorf_base.runtime, "bootstrap", fake_bootstrap)
...
def test_local_runner_tears_down_on_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(mysorf_base.runtime, "bootstrap", lambda overrides=None: DummyContext())
...
def test_local_runner_results_are_timezone_aware(monkeypatch: pytest.MonkeyPatch) -> None:
    import mysorf_base.runtime
    ...
    monkeypatch.setattr(mysorf_base.runtime, "bootstrap", lambda overrides=None: DummyContext())
    runner = LocalRunner(base_overrides=[])
    summary = runner.run([["lr=0.1"]], lambda ctx, params: {"loss": 1.0})

    assert summary.results
    assert all(result.created_at.tzinfo == UTC for result in summary.results)


def test_local_runner_cleans_up_temp_summary_file(tmp_path: Path) -> None:
    ...
    artifact_manager.save.side_effect = fake_save
    runner = LocalRunner(base_overrides=[], artifact_manager=artifact_manager)
    summary = runner.run([], lambda ctx, params: {"loss": 1.0})
...
def test_property_grid_strategy_count(space: SearchSpace) -> None:
    from mysorf_base.sweeps.core.strategies import _param_values
