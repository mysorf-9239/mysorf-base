...
from mysorf_base.logging import NullLogger, build_logger, parse_logging_config
...
def test_build_console_logger_from_composed_section() -> None:
    cfg = compose_typed_config()
    logger = build_logger(cfg.logging, name="mysorf-base.test")
...
def test_build_file_logger_creates_log_file(tmp_path: Path) -> None:
    path = tmp_path / "logs" / "mysorf-base.log"
    ...
    logger = build_logger(cfg, name="mysorf-base.file")
    logger.info("written")
