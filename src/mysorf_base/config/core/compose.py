"""Hydra config composition API for the mysorf-base config subsystem."""
...
def _config_dir() -> str:
    ...
    1. ``MYSORF_BASE_CONFIG_DIR`` environment variable — explicit override.
    2. Repository root ``conf/`` — four levels above this file in the `src/`
       layout, used during
       development and editable installs.
    3. Bundled ``conf/`` inside the installed package at
       ``<site-packages>/mysorf_base/conf/``.

    Returns:
        Absolute path to the resolved ``conf/`` directory.
    """
    env_override = os.environ.get("MYSORF_BASE_CONFIG_DIR")
    if env_override:
        return str(Path(env_override).expanduser().resolve())

    repo_conf = Path(__file__).resolve().parents[4] / "conf"
    if repo_conf.is_dir():
    ...
    raise FileNotFoundError("Unable to resolve a mysorf-base conf/ directory.")
...
def _candidate_env_files() -> list[Path]:
    ...
    explicit_env = os.environ.get("MYSORF_BASE_ENV_FILE")
    if explicit_env:
        candidates.append(Path(explicit_env).expanduser().resolve())

    workspace_root = os.environ.get("MYSORF_BASE_WORKSPACE_ROOT")
    if workspace_root:
    ...
    *,
    ...
    Args:
        ...
        resolve: When ``True``, OmegaConf interpolations are resolved before
            conversion.
        validate: When ``True``, :func:`~mysorf_base.config.validate_config` is
...
def _redact_recursive(node: object) -> None:
    """Recursively mask values whose key matches a sensitive-key pattern."""
...
def redact_secrets(cfg: DictConfig | AppConfig) -> str:
    """Render the config as YAML with sensitive values replaced by a mask.

    Any dict key whose name (case-insensitive) matches one of
    ``api_key``, ``token``, ``secret``, ``password``, or ``credential``
    is replaced by ``***REDACTED***`` at any nesting depth.
