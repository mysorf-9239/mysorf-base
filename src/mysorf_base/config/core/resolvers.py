"""OmegaConf custom resolvers for the mysorf-base config subsystem."""
...
def _workspace_root() -> str:
    ...
    Reads ``MYSORF_BASE_WORKSPACE_ROOT`` when set; falls back to the current
    working directory.

    Returns:
        Absolute path string for the workspace root.
    """
    candidate = os.environ.get("MYSORF_BASE_WORKSPACE_ROOT")
    if candidate:
