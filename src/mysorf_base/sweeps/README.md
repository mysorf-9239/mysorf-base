...
`mysorf_base.sweeps` standardizes hyperparameter search for ML and research workflows. It generates parameter combinations
...
└── backends/
    ...
    ├── null.py              # NullSweepRunner — disabled/no-op execution
...
Supported backends: `disabled`, `local`, `wandb`
...
When `enabled: false`, use `backend: disabled` to select the null-runner path explicitly.
...
```python
from mysorf_base.sweeps import SearchSpace, CategoricalParam, IntegerParam, FloatParam
...
```python
from mysorf_base.sweeps import build_sweep_runner, SweepsConfig
from mysorf_base.sweeps.core.strategies import GridStrategy
...
| `skip_reason`               | `str \| None`             | Why a sweep returned no trials intentionally                     |
...
When sweeps are disabled through config, `run_sweep(...)` returns an empty `SweepSummary`
with `skip_reason="sweeps disabled by configuration"` and emits a warning through the
provided runtime logger.
