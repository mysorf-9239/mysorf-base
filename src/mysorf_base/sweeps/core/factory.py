...
if TYPE_CHECKING:
    ...
    from mysorf_base.runtime.core.schema import RuntimeContext
...
def build_sweep_runner(
    ...
    if not config.enabled or config.backend == "disabled":
        from ..backends.null import NullSweepRunner

        return NullSweepRunner(skip_reason="sweeps disabled by configuration")
    ...
    raise ValueError(
        "Unsupported sweeps backend: "
        f"{config.backend!r}. Valid options: ['disabled', 'local', 'wandb']"
    )
...
def run_sweep(
    ...
    *params* rather than as Hydra overrides, allowing arbitrary keys that are
    ...
    validate_sweeps_config(config)
    if not config.enabled or config.backend == "disabled":
        skip_reason = "sweeps disabled by configuration"
        ctx.logger.warning(skip_reason)
        return SweepSummary(results=[], skip_reason=skip_reason)
