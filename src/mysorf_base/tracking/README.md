...
`mysorf_base.tracking` provides experiment-style tracking for ML and research workflows. It defines a thin abstraction over tracking backends so callers can log run metadata, metrics, and artifacts without coupling to a specific provider.
...
```python
from mysorf_base.tracking import (
    TrackingConfig,
    NullTracker,
    build_tracker,
...
```python
from mysorf_base.tracking import build_tracker, TrackingConfig
...
wandb:
  ...
  project: ${oc.env:WANDB_PROJECT,mysorf-base}
