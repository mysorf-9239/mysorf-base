...
if TYPE_CHECKING:
    from mysorf_base.config.core.schema import PathsSection
...
def build_artifact_manager(
    ...
    if cfg.backend == "s3":
        cache_dir = cfg.cache_dir if cfg.cache_dir is not None else paths.cache_dir
        return S3Backend(
    ...
    if cfg.backend == "gcs":
        ...
        return GCSBackend(
            ...
            run_id=run_id,
            tracker=tracker,
            project=cfg.gcs_project,
        )

    raise ValueError(
        f"Unsupported artifacts backend: {cfg.backend!r}. "
        f"Valid options: {sorted({'local', 'disabled', 's3', 'gcs'})}"
    )
