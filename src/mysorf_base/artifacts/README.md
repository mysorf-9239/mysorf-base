...
`mysorf_base.artifacts` provides a structured abstraction for saving, loading, listing, deleting, and resolving paths for ML
...
| `register_on_save_hook` | `(hook) -> None`                                         | Register an observer called after successful saves          |
| `finalize`       | `() -> None`                                                    | Release backend lifecycle resources                         |
...
```python
from mysorf_base.artifacts import build_artifact_manager, ArtifactsConfig
...
Artifact managers support lightweight post-save observers through `register_on_save_hook(...)`.
This allows downstream runtime or engine layers to react to persisted artifacts without changing
the `save(...)` call signature.

Hook contract:

- hooks run after a successful save returns an `ArtifactRecord`
- hooks receive exactly that `ArtifactRecord`
- hook failures are suppressed and logged as warnings
- hooks should be side-effect aware and fast

## Finalize contract

`finalize()` is owned by runtime teardown, not by downstream call sites.

- it should be safe to call during context exit
- it should be idempotent when practical
- backends may raise, but runtime teardown will suppress and warn
- local and null backends may implement it as a no-op
